...
public class ElasticComparisonNodeInterpreter
    ...
    private static QueryBuilder createEqualQuery(
            final ComparisonNode comparisonNode) {
    ...
    private static QueryBuilder createNotEqual(
            final ComparisonNode comparisonNode) {
        final String firstArg = firstArg(comparisonNode);
        final String selector = comparisonNode.getSelector();
    ...
    private static QueryBuilder createInQuery(
            final ComparisonNode comparisonNode) {
        return termsQuery(comparisonNode.getSelector(),
                comparisonNode.getArguments());
    }

    private static QueryBuilder createNotInQuery(
            final ComparisonNode comparisonNode) {
        final QueryBuilder termsQuery =
                termsQuery(comparisonNode.getSelector(),
                        comparisonNode.getArguments());
        return boolQuery().mustNot(termsQuery);
    }

    private static QueryBuilder createGreaterThanQuery(
            final ComparisonNode comparisonNode) {
        return rangeQuery(comparisonNode.getSelector()).gt(
                firstArg(comparisonNode));
    }

    private static QueryBuilder createGreaterThanOrEqualQuery(
            final ComparisonNode comparisonNode) {
        return rangeQuery(comparisonNode.getSelector()).gte(
                firstArg(comparisonNode));
    }

    private static QueryBuilder createLessThanQuery(
            final ComparisonNode comparisonNode) {
        return rangeQuery(comparisonNode.getSelector()).lt(
                firstArg(comparisonNode));
    }

    private static QueryBuilder createLessThanOrEqualQuery(
            final ComparisonNode comparisonNode) {
        return rangeQuery(comparisonNode.getSelector()).lte(
                firstArg(comparisonNode));
    }
    ...
    public QueryBuilder interpret(final ComparisonNode comparisonNode) {
        final ComparisonOperatorProxy operator =
                ComparisonOperatorProxy.asEnum(comparisonNode.getOperator());
        switch (operator) {
            case NOT_EQUAL:
